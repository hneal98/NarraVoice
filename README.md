...
## NarraVoice vs. Cloud TTS (ElevenLabs, OpenAI, etc.)

| Feature | NarraVoice (Local & Offline) | Cloud-Based TTS Engines |
| :--- | :--- | :--- |
| **Cost** | 💸 **100% Free** — No subscriptions, no credit cards. | 💳 **Per-character billing** — Gets expensive fast for long books. |
| **Privacy** | 🔒 **Fully Private** — Text and audio never leave your PC. | 🌐 **Cloud Logging** — Sent to third-party corporate servers. |
| **Hardware** | 💻 **CPU-Optimized** — Designed to run smoothly on standard laptops. | ☁️ **Server Dependent** — Requires internet; subject to server outages. |
| **Dialogue Control** | 🎭 **Per-Line Presets** — Gutter colors track multi-character voice changes. | 🧩 **Manual Splitting** — Hard to manage shifting character tones in one file. |
| **Fine-Tuning** | 🛠️ **Smart IPA & Local Dictionaries** — Total manual control over heteronyms. | 🤖 **AI Guesswork** — Often mispronounces names/heteronyms with no bypass. |
...
Also see the [context document](./docs/NarraVoiceContext.md) for more technical information.
